...
const GumGumButton = () => (
  <motion.button
    ...
    className="px-6 py-2 md:px-8 md:py-3 bg-red-600 text-white font-black tracking-wider rounded-lg shadow-[0_5px_0_rgb(153,27,27)] active:shadow-none active:translate-y-[5px] text-sm md:text-base"
  >
    GOMU GOMU NO...
  </motion.button>
);
...
const LogiaLoader = () => (
  <div className="relative w-12 h-12 md:w-16 md:h-16 flex items-center justify-center bg-black/20 rounded-full">
    {[...Array(3)].map((_, i) => (
      <motion.div
        key={i}
...
const BariBariModal = () => {
  ...
  return (
    <div className="relative h-32 w-full flex items-center justify-center">
      <Button onClick={() => setIsOpen(!isOpen)} variant="outline" className="border-green-500 text-green-400 hover:bg-green-500/10 z-10 text-xs md:text-sm">
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <Shield className="w-10 h-10 md:w-12 md:h-12 text-green-400 drop-shadow-[0_0_10px_rgba(74,222,128,0.8)]" />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
  ...
  return (
    <div className="relative w-32 h-32 flex items-center justify-center">
      ...
      <AnimatePresence>
        {active && (
          <motion.div
            ...
            animate={{ scale: 1, opacity: 1 }}
            exit={{ scale: 0, opacity: 0 }}
            className="absolute w-28 h-28 md:w-32 md:h-32 rounded-full border border-cyan-400/30 bg-cyan-400/10 pointer-events-none"
          />
        )}
      </AnimatePresence>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.9 }}
        className="relative z-10 w-10 h-10 md:w-12 md:h-12 bg-cyan-600 rounded-full flex items-center justify-center text-white shadow-lg shadow-cyan-500/50"
      >
        {active ? <X size={18} /> : <Menu size={18} />}
      </motion.button>
      <span className="absolute -bottom-4 md:-bottom-6 text-[8px] md:text-[10px] text-cyan-300 font-mono">{active ? "ROOM: ACTIVE" : "ROOM: OFF"}</span>
    </div>
  );
};
...
export function DevilFruitLibrary() {
  ...
  return (
    <div className="relative w-full">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-end justify-between mb-8 md:mb-10 gap-4 md:gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 md:p-3 bg-purple-900/50 rounded-xl border border-purple-500/30 shadow-[0_0_15px_rgba(168,85,247,0.3)]">
              <Sparkles className="w-6 h-6 md:w-8 md:h-8 text-purple-400 animate-pulse" />
            </div>
            <div>
              <h3 className="text-3xl md:text-4xl text-transparent bg-clip-text bg-gradient-to-r from-purple-200 to-purple-400 font-black tracking-tight" style={{ fontFamily: 'serif' }}>
                Devil Fruit Lab
              </h3>
              <p className="text-purple-300/60 text-xs md:text-sm font-mono tracking-widest mt-1">
        ...
        <div className="flex flex-wrap gap-1 bg-black/40 p-1 rounded-lg border border-purple-500/20 w-full md:w-auto">
          {['ALL', 'PARAMECIA', 'LOGIA', 'SPECIAL'].map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f as any)}
              className={`flex-1 md:flex-none px-3 py-1.5 text-[10px] font-bold rounded-md transition-all whitespace-nowrap ${filter === f ? 'bg-purple-600 text-white shadow-lg' : 'text-purple-300/50 hover:text-purple-200'}`}
            >
              {f}
            </button>
          ))}
        ...
        <AnimatePresence mode="popLayout">
          {filteredFruits.map((fruit) => {
            ...
            return (
              ...
              >
                {/* Card Header */}
                <div className="px-4 py-3 md:px-5 md:py-4 flex items-center justify-between border-b border-white/5 bg-[#1a1025]">
                  <div className="flex items-center gap-3">
                     <div className={`w-8 h-8 md:w-10 md:h-10 rounded-lg bg-gradient-to-br ${fruit.color} flex items-center justify-center shadow-lg`}>
                        <fruit.icon className="w-4 h-4 md:w-5 md:h-5 text-white mix-blend-overlay" />
                     </div>
                     <div>
                        <h4 className="text-sm md:text-base font-bold text-white">{fruit.name}</h4>
                        <span className="text-[9px] md:text-[10px] text-white/40 uppercase tracking-widest font-mono">{fruit.type} TYPE</span>
                     </div>
                  </div>
                  ...
                  <div className="flex bg-black/40 rounded-lg p-1 border border-white/5">
                      <button 
                        ...
                        className={`px-2 py-1 md:px-3 md:py-1.5 rounded flex items-center gap-1.5 md:gap-2 text-[10px] font-bold transition-all ${!isCodeMode ? 'bg-purple-600 text-white shadow' : 'text-white/40 hover:text-white'}`}
                      >
                      ...
                      <button 
                        ...
                        className={`px-2 py-1 md:px-3 md:py-1.5 rounded flex items-center gap-1.5 md:gap-2 text-[10px] font-bold transition-all ${isCodeMode ? 'bg-purple-600 text-white shadow' : 'text-white/40 hover:text-white'}`}
                      >
                        <Terminal className="w-3 h-3" /> <span className="hidden sm:inline">CODE</span>
                      </button>
                  </div>
                </div>

                {/* Card Body */}
                <div className="flex-1 relative bg-black/60 min-h-[220px] md:min-h-[240px] flex flex-col">
                      ...
                      {!isCodeMode ? (
                        ...
                        >
                           <div className="flex-1 overflow-auto custom-scrollbar p-3 md:p-4">
                              <code className="text-[10px] md:text-[11px] font-mono text-emerald-400 whitespace-pre leading-relaxed">
                                 {fruit.code}
                              </code>
                           </div>
                           <div className="p-3 border-t border-white/10 bg-[#160b25]">
                              <Button
                                ...
                                className={`w-full h-8 md:h-9 text-xs font-bold uppercase tracking-wider ${copiedId === fruit.id ? 'bg-green-600 hover:bg-green-600' : 'bg-purple-900/50 hover:bg-purple-800 text-purple-200'}`}
                              >
                                {copiedId === fruit.id ? <><Check className="w-4 h-4 mr-2"/> Copied</> : <><Copy className="w-4 h-4 mr-2"/> Copy Code</>}
                              </Button>
                           </div>
                        </motion.div>
                      )}
      ...
      <div className="mt-8 md:mt-12 p-4 md:p-6 rounded-2xl bg-gradient-to-r from-purple-900/40 to-indigo-900/40 border border-purple-500/30 text-center relative overflow-hidden">
