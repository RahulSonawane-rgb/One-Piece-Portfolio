...
function DenDenMushi() {
  ...
  useEffect(() => {
    ...
    const wander = async () => {
      ...
      const nextXPercent = 5 + Math.random() * 80; 
  ...
  return (
    <div className="relative w-full h-24 mt-8 overflow-visible">
      {/* Visual Floor Line */}
      ...
      >
        ...
        >
           ...
           {/* =========================================
              NEW LAYER: SLIME TRAIL (THIN LIQUID)
             ========================================= */}
           {/* This layer sits under the body, blurred to look wet */}
           <motion.div 
          ...
          {/* The "Skirt" (Wavy bottom edge - Expanded width) */}
          <motion.div
             // Increased width to match the new body length
          ...
          {/* =========================================
              LAYER 2: THE SHELL (DEEP CARVED LOOK)
             ========================================= */}
              ...
              <div className="absolute top-1/2 left-2/4 transform -translate-x-1/2 -translate-y-1/2 w-14 h-14 bg-gray-100 rounded-full border-[3px] border-gray-400 shadow-lg flex items-center justify-center">
                   <div className="w-10 h-10 rounded-full border-[4px] border-gray-300 bg-gradient-to-b from-white to-gray-200 relative shadow-inner">
                      {/* {[0, 60, 120, 180, 240, 300].map((deg, i) => (
                        <div key={i} className="absolute w-1.5 h-1.5 bg-black/70 rounded-full shadow-sm" 
                             style={{ transform: `rotate(${deg}deg) translate(12px)` }} />
                      ))} */}
                   </div>
              </div>
          </div>
          ...
          {/* =========================================
              LAYER 3: THE HEAD & FACE (DETAILED SOCKETS)
             ========================================= */}
          <div className="absolute bottom-14 left-4 w-20 h-24">
              ...
              <div className="absolute top-[-8px] left-[-30px] w-10 h-10 bg-rose-300 rounded-full shadow-[inset_0_2px_4px_rgba(0,0,0,0.3)] flex items-center justify-center -rotate-12 z-0">
                  <div className="w-8 h-8 bg-white rounded-full border border-gray-300 overflow-hidden flex flex-col justify-center items-center relative">
                       <div className="absolute top-0 w-full h-[55%] bg-rose-300/90 border-b-2 border-rose-500 shadow-sm" /> {/* Eyelid */}
                       <div className="w-2.5 h-2.5 bg-black rounded-full mt-4" /> {/* Pupil */}
                  </div>
              </div>

              {/* Right Eye (Deep Socket) */}
